...
const TABS: { id: TabId; icon: React.ElementType; gradient: string }[] = [
  { id: 'clients',        icon: Users,    gradient: 'from-blue-500 to-primary-600'   },
  { id: 'devis',          icon: FileText, gradient: 'from-teal-500 to-emerald-600'   },
  { id: 'bonsLivraison',  icon: Truck,    gradient: 'from-amber-500 to-orange-500'   },
  { id: 'factures',       icon: Receipt,  gradient: 'from-purple-500 to-violet-600'  },
  { id: 'analytics',      icon: BarChart3,gradient: 'from-orange-500 to-amber-600'  },
]
...
const STATUS_BADGE: Record<string, string> = {
  actif:     'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300',
  enAttente: 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300',
  accepte:   'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300',
  envoye:    'bg-blue-100 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300',
  vu:        'bg-purple-100 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300',
  brouillon: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400',
  payee:     'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300',
  partielle: 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300',
  enRetard:  'bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300',
  envoyee:   'bg-blue-100 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300',
  livre:     'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300',
  facture:   'bg-violet-100 dark:bg-violet-900/60 text-violet-700 dark:text-violet-300',
}
  ...
  return (
    <section ref={ref} className="py-24 sm:py-28 overflow-hidden bg-slate-50 dark:bg-slate-900/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <div className={cn('text-center mb-12 transition-all duration-700', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8')}>
        ...
        <div className={cn('mb-10 transition-all duration-700 delay-100', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8')}>
          <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap sm:justify-center scrollbar-none">
            {TABS.map(({ id, icon: Icon }, i) => (
              <button
                key={id}
                onClick={() => setActiveIdx(i)}
                className={cn(
                  'flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 whitespace-nowrap flex-shrink-0',
                  activeIdx === i
        ...
        <div className={cn('grid lg:grid-cols-2 gap-10 items-center transition-all duration-700 delay-200', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8')}>
          ...
          {/* Left: description */}
          <div>
            <div className={cn('w-12 h-12 rounded-2xl flex items-center justify-center mb-6 shadow-lg bg-gradient-to-br', tab.gradient)}>
          ...
          <div className="relative">
            ...
            <div className="relative card rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700">
              ...
              <div className="p-4 bg-white dark:bg-slate-900">
                ...
                {/* Column headers */}
                <div className="grid grid-cols-4 gap-2 px-3 py-2 mb-1 text-[10px] font-semibold text-slate-400 uppercase tracking-wide">
                ...
                <div className="space-y-1">
                  {SCREEN_DATA[tab.id].map((row, i) => (
                    <div key={i} className="grid grid-cols-4 gap-2 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group">
