...
interface GroupeDepense { taux: number; montantHT: number; tva: number; count: number }
    ...
    <div className="space-y-5 pb-8">
      ...
      {result && !loading && (
        <div className="space-y-4">
          ...
          <div className="card rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
              <h2 className="font-bold text-slate-900 dark:text-white text-sm">{t('declarationsTva.deductibleTitle')}</h2>
              <p className="text-xs text-slate-400 mt-0.5">{t('declarationsTva.deductibleSubtitle')}</p>
            </div>

            {result.groupesDepenses.length === 0 ? (
              <div className="px-5 py-10 text-center text-sm text-slate-400">{t('declarationsTva.noDepenses')}</div>
            ) : (
              <>
                <div className="hidden sm:grid grid-cols-3 gap-4 px-5 py-2 bg-slate-50 dark:bg-slate-800/60 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                  <span>{t('declarationsTva.taux')}</span>
                  <span className="text-right">{t('declarationsTva.montantHTDepenses')}</span>
                  <span className="text-right">{t('declarationsTva.tvaDeductible')}</span>
                </div>

                {result.groupesDepenses.map((g, i) => (
                  <div key={g.taux} className={cn(
                    'grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 px-5 py-3 border-b border-slate-100 dark:border-slate-800',
                    i % 2 === 0 ? '' : 'bg-slate-50/50 dark:bg-slate-800/20',
                  )}>
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">{g.count} dépense{g.count > 1 ? 's' : ''}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{fmt(g.montantHT)} MAD</p>
                      <p className="text-xs text-slate-400 sm:hidden">Base HT</p>
                    </div>
                    <div className="text-right col-span-2 sm:col-span-1">
                      <p className="text-sm font-bold text-amber-600 dark:text-amber-400">{fmt(g.tva)} MAD</p>
                      <p className="text-xs text-slate-400 sm:hidden">TVA déductible</p>
                    </div>
                  </div>
                ))}

                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 px-5 py-4 bg-amber-50 dark:bg-amber-950/20 border-t-2 border-amber-200 dark:border-amber-800">
                  <div>
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Total</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-black text-slate-900 dark:text-white">{fmt(result.totalBaseHTDepenses)} MAD</p>
                    <p className="text-xs text-slate-500">{t('declarationsTva.totalBaseHTDepenses')}</p>
                  </div>
                  <div className="text-right col-span-2 sm:col-span-1">
                    <p className="text-base font-black text-amber-700 dark:text-amber-300">{fmt(result.totalTVADeductible)} MAD</p>
                    <p className="text-xs text-slate-500">{t('declarationsTva.totalTVADeductible')}</p>
                  </div>
                </div>
              </>
            )}
          ...
          {/* TVA Nette à Payer */}
          <div className="card rounded-2xl overflow-hidden border-2 border-primary-200 dark:border-primary-800">
            <div className="px-5 py-4 border-b border-primary-100 dark:border-primary-900 bg-primary-50 dark:bg-primary-950/30">
              <h2 className="font-bold text-primary-900 dark:text-primary-100 text-sm">{t('declarationsTva.netteTitle')}</h2>
              <p className="text-xs text-primary-600 dark:text-primary-400 mt-0.5">{t('declarationsTva.netteSubtitle')}</p>
            </div>
            <div className="px-5 py-5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-slate-400">{t('declarationsTva.collecteeLabel')}</span>
                <span className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">+{fmt(result.totalTVA)} MAD</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-slate-400">{t('declarationsTva.deductibleLabel')}</span>
                <span className="text-sm font-semibold text-amber-700 dark:text-amber-400">−{fmt(result.totalTVADeductible)} MAD</span>
              </div>
              <div className="h-px bg-slate-200 dark:bg-slate-700" />
              <div className="flex items-center justify-between">
                <span className="text-base font-bold text-slate-900 dark:text-white">{t('declarationsTva.netteLabel')}</span>
                <span className={cn(
                  'text-xl font-black',
                  result.totalTVANette >= 0
                    ? 'text-primary-700 dark:text-primary-300'
                    : 'text-emerald-600 dark:text-emerald-400',
                )}>
                  {fmt(result.totalTVANette)} MAD
                </span>
              </div>
              <p className="text-xs text-slate-400 dark:text-slate-500 pt-1">{t('declarationsTva.netteNote')}</p>
            </div>
          </div>
