...
import {
  ...
  Mail, FileText, Receipt, AlertTriangle, Lock, Hash, ShieldCheck, HardDrive, Pen, Trash2,
} from 'lucide-react'
...
type Tab = 'profile' | 'company' | 'branding' | 'language' | 'theme' | 'notifications' | 'security' | 'billing' | 'dgi'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    settingsApi.getBranding().then(r => {
      ...
      setSavedSignature(d.signature ?? null)
  ...
  const saveSignature = async () => {
    if (!sigCanvasRef.current || sigCanvasRef.current.isEmpty()) {
      toastError('Signature vide', t('pages.settings.signatureModal.emptyError'))
      return
    }
    setSigSaving(true)
    try {
      const dataUrl = sigCanvasRef.current.toDataURL()
      await settingsApi.updateBranding({ signature: dataUrl })
      setSavedSignature(dataUrl)
      setShowSigCanvas(false)
      success(t('pages.settings.branding.signatureSaved'), '')
    } catch (err) { handleApiError(err) }
  ...
  const deleteSignature = async () => {
    setSigSaving(true)
    try {
      await settingsApi.updateBranding({ signature: null })
      setSavedSignature(null)
      setShowSigCanvas(false)
      success(t('pages.settings.branding.signatureDeleted'), '')
    } catch (err) { handleApiError(err) }
    finally { setSigSaving(false) }
  }
    ...
    switch (activeTab) {
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{t('pages.settings.branding.signatureTitle')}</p>
                </div>
                <div className="flex items-center gap-2">
                  {savedSignature && !showSigCanvas && (
                    <button
                      onClick={deleteSignature}
                      disabled={sigSaving}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition-all disabled:opacity-60"
                    >
                      <Trash2 className="w-3 h-3" />
                      {t('pages.settings.branding.signatureDelete')}
                    </button>
                  )}
                  <button
                    onClick={() => { setShowSigCanvas(v => !v); sigCanvasRef.current?.clear() }}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-primary-600 dark:text-primary-400 border border-primary-200 dark:border-primary-800 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-950/30 transition-all"
                  >
                    <Pen className="w-3 h-3" />
                    {showSigCanvas ? t('pages.settings.branding.signatureClear') : (savedSignature ? t('pages.settings.branding.signatureDraw') : t('pages.settings.branding.signatureDraw'))}
                  </button>
                </div>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{t('pages.settings.branding.signatureSubtitle')}</p>

              {/* Saved preview */}
              {savedSignature && !showSigCanvas && (
                <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-900 p-3">
                  <p className="text-xs text-slate-400 dark:text-slate-500 mb-2 font-semibold uppercase tracking-wide">Signature actuelle</p>
                  <img src={savedSignature} alt="Signature" className="max-h-20 object-contain" style={{ filter: 'none' }} />
                </div>
              )}
              ...
              {showSigCanvas && (
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">{t('pages.settings.branding.signatureHint')}</p>
                  <div className="border-2 border-dashed border-primary-300 dark:border-primary-700 rounded-xl overflow-hidden bg-white dark:bg-white">
                    <SignatureCanvas
                      ref={sigCanvasRef}
                      width={600}
                      height={150}
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    <button
                      ...
                      className="px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                    >
                      {t('pages.settings.branding.signatureClear')}
                    </button>
                    <button
                      onClick={saveSignature}
                      disabled={sigSaving}
                      className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-xl transition-all disabled:opacity-60"
                    >
                      {sigSaving && <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                      {t('pages.settings.branding.signatureSave')}
                    </button>
                  </div>
                </div>
              )}
              ...
              {!savedSignature && !showSigCanvas && (
                <p className="text-xs text-slate-400 dark:text-slate-500 italic">{t('pages.settings.branding.signatureEmpty')}</p>
              )}
            </div>
      ...
      case 'dgi':
        return (
          <div className="space-y-6">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white mb-1">{t('dgi.title')}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">{t('dgi.subtitle')}</p>
            </div>
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
              <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">{t('dgi.modeLabel')}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t('dgi.modeDesc')}</p>
                </div>
              </div>
